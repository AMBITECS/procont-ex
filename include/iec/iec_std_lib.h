...
//
// ----------------------------------------------------------------------------
// IEC 61131-3 standard function & FBs library
// ----------------------------------------------------------------------------
...
// Including <sys/time.h> broke all the other platforms.
// Might need to check for an alternative for Alpine
// Reverted to exclude this.
//#include <sys/time.h>
...
// Keep this macro extention step to let sfx(__VA_ARGS__) change into L or LL
...
// ----------------------------------------------------------------------------
//    NOTE: The following function was turned into a macro, so it could be used
//    to initialize the initial value of TOD (TIME_OF_DAY) variables
//    NOTE: many (but not all) of the same comments made regarding __time_to_timespec()
//    are also valid here, so go and read those comments too!
// ----------------------------------------------------------------------------
...
//  ts.tv_sec  = sign * (ts_time_t)total_sec;
//  ts.tv_nsec = sign * (ts_time_t)((total_sec - ts.tv_sec)*1e9);
//  return ts;
//}
// ----------------------------------------------------------------------------
#define __time_to_timespec(sign,mseconds,seconds,minutes,hours,days) \
    ((IEC_TIMESPEC) {\
        /*tv_sec  =*/ (static_cast<ts_time_t>   (((sign>=0)?1:-1)*((((long double)days*24 + (long double)hours)*60 + (long double)minutes)*60 + (long double)seconds + (long double)mseconds/1e3))), \
        /*tv_nsec =*/ (static_cast<ts_time_t>   (( \
...
//  ts.tv_sec  = (ts_time_t) total_sec;
//  ts.tv_nsec = (ts_time_t) ((total_sec - ts.tv_sec)*1e9);
//  return ts;
//}
// ----------------------------------------------------------------------------
#define __tod_to_timespec(seconds,minutes,hours) \
      ((IEC_TIMESPEC) {\
          /*tv_sec  =*/ (static_cast<ts_time_t>   ((((long double)hours)*60 + (long double)minutes)*60 + (long double)seconds)), \
          /*tv_nsec =*/ (static_cast<ts_time_t>   (           (                                     \
...
static inline IEC_TIMESPEC __date_to_timespec(int day, int month, int year) {
  ...
  if (month < 1 || month > 12) __iec_error();

  yday = __mon_yday[__isleap(year)][month - 1] + day;
  if (yday > __mon_yday[__isleap(year)][month]) __iec_error();
...
// ----------------------------------------------------------------------------
static inline IEC_TIMESPEC __dt_to_timespec(
        double seconds, double minutes, double hours,
...
static inline TIME __time_mul(TIME IN1, LREAL IN2) {
  ...
  TIME res = {static_cast<uint32_t>((long)s + ns.quot),
		      static_cast<uint32_t>((long)ns.rem + (long)(s_f - (LREAL)s) * 1000000000) };
  __normalize_timespec(&res);
  return res;
}
...
static inline TIME __time_div(TIME IN1, LREAL IN2) {
  LREAL s_f = IN1.tv_sec / IN2;
  auto s = (time_t)s_f;
  TIME res = {static_cast<ts_time_t>((long)s),
              static_cast<ts_time_t>((long)(IN1.tv_nsec / IN2 + (s_f - (LREAL)s) * 1000000000)) };
  __normalize_timespec(&res);
  return res;
}
...
static inline LREAL __string_to_real(STRING IN) {
    ...
    while(--l > 0 && IN.body[l] != '.');
    return (l != 0)
        ? atof((const char *)&IN.body)
        : (LREAL)__pstring_to_sint(&IN);
}
...
static inline TIME __real_to_time(LREAL IN) {
    return (TIME) {
        static_cast<ts_time_t>(IN),
        static_cast<ts_time_t>((IN - (LINT)IN) * 1000000000)
    };
}
...
static inline TIME __string_to_time(STRING IN) {
    ...
    if (l != 0) {
        ...
        return  (TIME){
            static_cast<ts_time_t>((long)IN_val),
            static_cast<ts_time_t>((long)(IN_val - (LINT)IN_val)*1000000000)};
    } else {
        return  (TIME){
            static_cast<ts_time_t>((long)__pstring_to_sint(&IN)),
            0};
    }
}
...
static inline STRING __time_to_string(TIME IN) {
    ...
    if(!days.rem && IN.tv_nsec == 0){
        res.len = snprintf((char*)&res.body, STR_MAX_LEN, "T#%dd",
                           days.quot);
    }else{
        div_t hours = div(days.rem, SECONDS_PER_HOUR);
        if (!hours.rem && IN.tv_nsec == 0) {
            res.len = snprintf(
                    (char*)&res.body, STR_MAX_LEN, "T#%dd%dh",
                    days.quot, hours.quot);
        } else {
            div_t minuts = div(hours.rem, SECONDS_PER_MINUTE);
            if (!minuts.rem && IN.tv_nsec == 0) {
                res.len = snprintf(
                        (char*)&res.body, STR_MAX_LEN, "T#%dd%dh%dm",
                        days.quot, hours.quot, minuts.quot);
            } else {
                if (IN.tv_nsec == 0) {
                    res.len = snprintf((char*)&res.body, STR_MAX_LEN, "T#%dd%dh%dm%ds",
                                       days.quot, hours.quot, minuts.quot, minuts.rem);
                } else {
                    res.len = snprintf((char*)&res.body, STR_MAX_LEN, "T#%dd%dh%dm%ds%gms",
                                       days.quot, hours.quot, minuts.quot, minuts.rem, (LREAL)IN.tv_nsec / 1000000);
                }
            }
        }
    }
