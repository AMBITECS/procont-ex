...
public:
    ...
    CConnectLine*    add_new_line(CConnectorPin *dragged_pin, CConnectorPin *target_pin);

private:
    CLadder * m_ladder{nullptr};
    CConnectorPin    * m_dragged_pin{nullptr};
    CConnectorPin    * m_target_pin{nullptr};
    int object_bottom_max{0};

    /** @brief рисует горизонтальную линию от пина до области ЗА bound_text_rect */
    static QLine    first_line(CConnectorPin *pin, CConnectorPin *opp);
    ...
    static int     index_of(std::vector<CConnectorPin*> *pins_array, CConnectorPin *pin);
};
