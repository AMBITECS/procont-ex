...
//
//    for (const auto& [addr, key] : subscriptions_) {
//        // Проверка изменения через addr.category() внутри isProxyChanged
//        if (::isProxyChanged(addr)) {
//            // Получаем значение (категория определяется внутри addr)
//            uint64_t value = ::getProxyValue(addr);
...
//void RegClient::read(std::vector<ItemData>& items) {
//    for (auto& item : items) {
//        item.data = ::getProxyValue(item.addr); // Категория из item.addr.category()
//    }
//}
//
//// Запись данных
//void RegClient::write(const std::vector<ItemData>& items) {
//    for (const auto& item : items) {
//        ::setProxyValue(item.addr, item.data); // Категория из item.addr.category()
//    }
//}
